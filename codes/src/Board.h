...
public:
    ...
    inline std::vector<std::vector<Insect*>>* getGrid(); // 返回蜂巢，用于判断是否可以获取昆虫等操作
    ...
    std::vector<Position> getValidPositions(const Insect&); // 被用于isValidMove判断，计算某棋盘/用户包里的昆虫可以在棋盘上走的位置
    ...
    // 与昆虫操作相关
    void placePiece(Insect* insect, const Position& pos); // 应当被playerControl调用，不能自己使用
