...
public:
    ...
    bool checkGameEnd() const; // 每次昆虫操作后都要检查一遍
    ...
    [[nodiscard]] std::vector<Position> getValidPositions(const Insect&) const; // 被用于isValidPosition判断，计算某棋盘/用户包里的昆虫可以在棋盘上走的位置
    ...
    static bool isValidPosition(Insect* insect, const Position& target); // 对于昆虫在这上边是否有效，应当在内部调用昆虫自己的判定
