...
public:
        ...
        void addPiece(Board* board, Insect* insect, const Position &position) const; // 添加一个旗子到棋盘上
        void makeMove(Board* board, Insect* insect, const Position& position) const; // 玩家移动棋子的入口函数
        void chooseInsect(const Board* board, const Insect& insect) const; // 玩家点击了昆虫（无论是昆虫包里的还是棋盘上的）
